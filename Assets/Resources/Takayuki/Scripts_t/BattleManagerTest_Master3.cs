...
public class BattleManagerTest_Master3 : MonoBehaviour
{
    // UIコンポーネントの参照
    ...
    public EnemyDataTest_Master2 currentEnemy; // 現在の敵
    public Image EnemyImage; // 敵画像を表示するUI

    private PlayerTest_Master2 player;
    private List<EnemyDataTest_Master2> enemyList;
    ...
    void Start()
    {
        ...
        // シングルトンから敵データを取得
        currentEnemy = EnemyDataManagerTest_Master2.Instance.currentEnemy;
    ...
    {
        player = new PlayerTest_Master2("Hero", PlayerDataManagerTest_Master2.Instance.MaxHP, PlayerDataManagerTest_Master2.Instance.ATK);
        player.HP = PlayerDataManagerTest_Master2.Instance.CurrentHP;
    }
    ...
    {
        string filePath = Application.dataPath + "/Resources/Takayuki/Scripts_t/Json/area4_monsters.json";
    ...
    private void SelectRandomEnemy()
    {
        if (enemyList != null && enemyList.Count > 0)
        {
            ...
            // 敵画像を読み込む
            Sprite enemySprite = Resources.Load<Sprite>("Takayuki/Images_t/Valsabros");
            if (enemySprite != null)
            {
            ...
            // シングルトンに保存
            EnemyDataManagerTest_Master2.Instance.currentEnemy = currentEnemy;
    ...
    IEnumerator PlayerActionCoroutine(string action)
    {
        ...
        else if (action == "heal")
        {
            ...
            else
            {
                ...
                PlayerDataManagerTest_Master2.Instance.UpdateHP(player.HP); // HPを保存
    ...
    private IEnumerator LogAction(string message, System.Action onComplete = null)
    {
        ...
        yield return StartCoroutine(DisplayText(message)); // 1文字ずつ表示
    ...
    void EndBattle(string result)
    {
        ...
        {
            PlayerDataManagerTest_Master2.Instance.UpdateHP(player.HP); // 現在のHPを保存
            StartCoroutine(FadeAndTransitionToScene("EndRollTest_Master2")); // 勝利・逃げる成功→フィールドに移行
        }
        else if (result == "lose")
        {
            PlayerDataManagerTest_Master2.Instance.ResetData(); // 敗北時にデータをリセット
            StartCoroutine(FadeAndTransitionToScene("GameOverTest_Master2")); // 敗北→GameOver画面に移行
        }
    }
